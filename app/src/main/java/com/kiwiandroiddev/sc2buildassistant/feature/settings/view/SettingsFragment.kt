...
import com.kiwiandroiddev.sc2buildassistant.feature.settings.data.sharedpreferences.SettingKeys.*
...
class SettingsFragment : PreferenceFragment(), SettingsView, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onSharedPreferenceChanged(sharedPref: SharedPreferences, key: String) {
        ...
        if (key == KEY_GAME_SPEED) {
            // Set summary to be the user-description for the selected value
            val speedValues = this.resources.getStringArray(R.array.pref_game_speed_text)
            val index = Integer.parseInt(sharedPref.getString(KEY_GAME_SPEED, "4"))
            val speed = speedValues[index]
            pref.summary = speed
        } else if (key == KEY_EARLY_WARNING) {
            ...
            pref.summary = summary
        } else if (key == KEY_ENABLE_TRACKING) {
            val myInstance = GoogleAnalytics.getInstance(activity)
            myInstance.appOptOut = !sharedPref.getBoolean(KEY_ENABLE_TRACKING, true)
        } else if (key == KEY_SHOW_ADS) {
            trackNewAdsPreferenceSelection(sharedPref)
        }
    }

    private fun trackNewAdsPreferenceSelection(sharedPref: SharedPreferences) {
        val showAds = sharedPref.getBoolean(KEY_SHOW_ADS, true)
